...
//code by Shadow Mario, seriously thanks for making Psych Engine open source :)
...
class NotesSubstate extends MusicBeatSubstate
{
	...
	override function update(elapsed:Float) {
		if(changingNote) {
			...
			{
				trace ('note colors: ' + FlxG.save.data.arrowHSV);
	...
	function changeSelection(change:Int = 0) {
		curSelected += change;
		if (curSelected < 0)
			curSelected = Math.round(FlxG.save.data.arrowHSV.length) - 1;
